...
################################################################################
#                     Gradient Boosting Movement Prediction                    #
#                  With Half Sell Trailing Stop Loss Strategy                  #
#                                                                              #
# This strategy predicts hourly and buys cryptocurrencies predicted to have an #
# upwards movement in the next hour. When buying, all available funds are used #
# to open a position and are split equally among the cryptocurrencies to buy.  #
# A trailing stop loss is applied for half sell orders (half of the available  # 
# assets of a cryptocurrency are sold once a 1.5% trailing loss is hit).       #
################################################################################

class CryptoLSTMStrategy(QCAlgorithm):
    def Initialize(self):
        self.model = self.LoadModel("/TOP_10_1h_MVMT_PRUNED_WARM_NEST_100.pkl") # Load model
    ...
    def Trade(self):
        ...
        for symbol in symbols_to_buy:
            self.buy_crypto_holdings(symbol, 1/len(symbols_to_buy))  # Adjust for price
