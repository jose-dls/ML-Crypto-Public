...
def train_model_incrementally(file_paths, folder_output, model_name, test_name=None, n_est=100, save_increments=True):
    """
    Incrementally trains a gradient boosting model that predicts the up or down movement of a cryptocurrency using crypto historical data.
    ...
    Args:
        ...
        folder_output: Target folder for the saved gradient boosting model.
        model_name: Name of saved gradient boosting model (should be .pkl)
        test_name: Filename to save simple test results. Leave as None to not save tests.
        n_est: Number of estimators for the gradient boosting model.
    ...
    model = GradientBoostingClassifier(n_estimators=n_est, random_state=17, warm_start=True)
    n_est_total = 0
    count = 0
    ...
    with tqdm(total=len(file_paths)) as pbar:
        for crypto in file_paths:
            count += 1
            if count == 1:
                n_est_total += floor(n_est * 0.2)
            elif count == 2:
                n_est_total += floor(n_est * 0.15)
            else:
                n_est_total += floor(n_est * (0.65 / len(file_paths)))
...
def main():
    parser = argparse.ArgumentParser(description='Train a gradient boosting model to predict cryptocurrency up and down movements.')
    ...
    parser.add_argument('--output_folder', type=str, default="./", help='Output folder to save gradient boosting data.')
    parser.add_argument('--model_name', type=str, default="model.pkl", help='Filename to save gradient boosting model (should be .pkl).')
    parser.add_argument('--test_name', type=str, default=None, help='Filename to save simple test results.')
    parser.add_argument('--n_estimators', type=int, default=100, help='Number of estimators for the gradient boosting model.')
